...
// Actions
import { agencyRegister, login } from "services/user/action";
...
class SignUp extends Component {
  ...
  componentDidMount = async () => {
    if (this.props.user) {
      const user = this.props.user.res;
      const newState = {
        ...this.state,
        values: {
          ...this.state.values,
  ...
  handleSignUp = async () => {
    ...
    this.props.agencyRegister(values).then((res) => {
      if (res) {         
          if (this.state.fromAdmin) {
            // this.props.history.push("/rc/dashboard");
            let loginStatus = { isVerified: true };
            this.props
              .login(values.username, values.password, loginStatus)
              .then((status) => {
