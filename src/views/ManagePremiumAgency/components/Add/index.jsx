...
const AddAgency = (props) => {
  ...
  const { t } = useTranslation(["managePremiumAgency", "common"]);
  ...
  let [values, setValues] = useState(initialAgencyState.values);
  const [isEditing, setEditing] = useState(false);
  const [errors, setErrors] = useState(initialAgencyState.errors);
  ...
  function handleConfirmation() {
    props.history.push({
      pathname: "/rc/manage-premium-agency",
    });
  }
            ...
            <Grid item xs={12} sm={6} md={4}>
              <div className={classes.policy}>
                <input
                  type="checkbox"
                  onChange={() =>
                    handleFieldChange("isPremium", !values.isPremium)
                  }
