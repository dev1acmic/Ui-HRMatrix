...
import { addUser, loadRoles } from "services/admin/action";
...
const AddNewUser = (props) => {
  ...
  function handleAddUser(async) {
    if (validateForm()) {
      ...
      props.addUser(values).then((status) => {
        setIsVerified(values.isVerified);
        let msg = "";
        if (values.id > 0) {
  ...
  function handleConfirmation() {
    props.history.push({
      pathname: "/rc/manage-user",
    });
  }

  function getMessage(id, status) {
    if (id > 0) {
      return t("succMsg.userUpdatedSuccessfully");
    } else {
      return t("common:succMsg.useraddedsuccessfully");
    }
    // if (isVerified) {
          ...
          <Grid container item spacing={3} className={classes.formContainer}>
            <Grid item xs={12}>
              <Typography variant="h1" className={classes.pageTitle}>
                {isEditing ? t("common:updateUser") : t("common:addnewuser")}
              </Typography>
            </Grid>
          </Grid>
          <Grid container item spacing={3} className={classes.formContainer}>
          ...
          <Grid container item spacing={3} className={classes.formContainer}>
            ...
            <Grid item xs={12} sm={6} md={4}>
              <FormControl variant="outlined" className={classes.formControl}>
                <FormLabel className={classes.formLabel}>{t("role")}</FormLabel>
                <Select
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(AddNewUser))
);
